...
def getArbOrders(X,Y,poolPrice,globalPrice,arbTrigger,swapFunction,arbCompetitionGauge):
  ...
  if swapFunction == "CPMM": orderAmtMultiplier = 0.5
  elif swapFunction == "ESPM": orderAmtMultiplier = 0.25+(0.5-0.25)*arbCompetitionGauge
...
def getRandomOrders(X,Y,poolPrice,globalPrice,randomOrderSize,numberOfRandomOrderPerDay,simBlockSize,secondsPerBlock):  

  XtoYNewSize = round(random.random()*numberOfRandomOrderPerDay/(24*60*60/(secondsPerBlock*simBlockSize)))
  YtoXNewSize = round(random.random()*numberOfRandomOrderPerDay/(24*60*60/(secondsPerBlock*simBlockSize)))
  XtoYNewOrders = [] # buying Y from X
  YtoXNewOrders = [] # selling Y for X

  for i in range(0,XtoYNewSize):
    orderPrice = globalPrice*(1+(int(random.random()*30)/10-0.5)*0.001)
    orderAmt = X*random.random()*randomOrderSize*2
    newOrder = {
      "orderPrice":orderPrice,
      "orderAmt":orderAmt,
    }
  ...
  for i in range(0,YtoXNewSize):
    orderPrice = globalPrice*(1+(int(random.random()*30)/10-0.5)*0.001)
    orderAmt = Y*random.random()*randomOrderSize*2
    newOrder = {
      "orderPrice":orderPrice,
      "orderAmt":orderAmt,
    }
...
def calculateMatchIncrease(X, Y, Wx,Wy,orderbook, swapFunction):
  ...
  for order in orderbook:
    ...
    if order["orderPrice"] < currentPrice:
      ...
      matchType, EX, EY, originalEX, originalEY, swapPrice, PoolY, transactAmt = calculateSwapIncrease(X,Y,Wx,Wy,orderbook, orderPrice, lastOrderPrice, swapFunction)
...
def calculateMatchDecrease(X, Y, Wx, Wy, orderbook,swapFunction):
  ...
  for order in orderbook:
    ...
    if order["orderPrice"] > currentPrice:
      ...
      matchType, EX, EY, originalEX, originalEY, swapPrice, PoolX, transactAmt = calculateSwapDecrease(X,Y,Wx,Wy,orderbook, orderPrice, lastOrderPrice, swapFunction)
...
def computePriceDirection(X, Y, Wx,Wy,poolPrice, orderbook,swapFunction):
  ...
  if priceDirection == "stay":
    ...
    matchType, swapPrice, EX, EY, originalEX, originalEY, PoolX, PoolY = calculateMatchStay(X, Y, poolPrice, orderbook)
    stayResult = [matchType, swapPrice, EX, EY, originalEX, originalEY, PoolX, PoolY]
    return stayResult

  elif priceDirection == "increase":

    matchType, swapPrice, EX, EY, originalEX, originalEY, PoolX, PoolY = calculateMatchIncrease(X, Y, Wx,Wy,orderbook,swapFunction)
    ...
    matchType, swapPrice, EX, EY, originalEX, originalEY, PoolX, PoolY = calculateMatchDecrease(X, Y, Wx, Wy, orderbook,swapFunction)
    ...
    for secondToken in range(1,numberOfReserveTokens):
      for globalPrice in globalPriceList:
        if globalPrice["tokenPair"] == "0/"+str(secondToken):
          globalPrice["globalPrice"].append(globalPrice["globalPrice"][-1]*getRandomChange(vol, secondsPerBlock*simBlockSize/(365*24*60*60),priceJumpPerDay, priceJumpMagnitude))
          break
  ...
  while height<int(simSeconds/secondsPerBlock/simBlockSize):
    ...
    for firstToken in range(0,numberOfReserveTokens-1):
      for secondToken in range(firstToken+1,numberOfReserveTokens):
        ...
        XtoYNewOrders, YtoXNewOrders, arbProfit = getArbOrders(X,Y,poolPrice,globalPrice,arbTrigger,swapFunction,arbCompetitionGauge)
    ...
    for firstToken in range(0,numberOfReserveTokens-1):
      for secondToken in range(firstToken+1,numberOfReserveTokens):
        ...
        # get random orders
        XtoYNewOrders, YtoXNewOrders = getRandomOrders(X,Y,poolPrice,globalPrice,randomOrderSize,(tradingVolumePerDay/randomOrderSize),simBlockSize,secondsPerBlock)
        if len(XtoYNewOrders)+len(YtoXNewOrders) > 0:
    ...
    for firstToken in range(0,numberOfReserveTokens-1):
      for secondToken in range(firstToken+1,numberOfReserveTokens):
        ...
        # add new orders
        XtoY, YtoX, lastOrderID = addOrders(XtoY, YtoX, XtoYNewOrders, YtoXNewOrders, lastOrderID, height, orderLifeSpanHeight, feeRate)

        # swap pre-calculation
        matchType, swapPrice, EX, EY, originalEX, originalEY, PoolX, PoolY = swapCalculation(X, Y, Wx, Wy, poolPrice, XtoY, YtoX,swapFunction)
...
def simulateAllParams(simNum):
  ...
  globalPriceList = getGlobalPriceList(vol, simSeconds, secondsPerBlock, simBlockSize,priceJumpPerDay, priceJumpMagnitude)

  # setup list of simulation parameters
  arbTriggerList = [0.01,0.015,0.02]
  tradingVolumePerDayList = [0.1,0.2,0.4]
  ...
  for arbTrigger in arbTriggerList:
    for tradingVolumePerDay in tradingVolumePerDayList:
      for arbCompetitionGauge in arbCompetitionGaugeList:
        params = [feeRate,arbTrigger, tradingVolumePerDay,randomOrderSize,vol,priceJumpPerDay, priceJumpMagnitude, arbCompetitionGauge]
        CPMMtokenReserves, CPMMarbTradingVolume, CPMMarbTotalProfit = simulation("CPMM",params, globalPriceList)
        ESPMtokenReserves, ESPMarbTradingVolume, ESPMarbTotalProfit = simulation("ESPM",params, globalPriceList)
        printStr = str(simNum)
        ...
        printStr += "/" + str(CPMMtokenReserves[0]["amount"]+globalPriceList[0]["globalPrice"][-1]*CPMMtokenReserves[1]["amount"])
        printStr += "/" + str(CPMMarbTradingVolume)
        printStr += "/" + str(CPMMarbTotalProfit)
        printStr += "/" + str(ESPMtokenReserves[0]["amount"]+globalPriceList[0]["globalPrice"][-1]*ESPMtokenReserves[1]["amount"])
